...
static SDispatchResult swapWindows(const std::string& first, const std::string& second){
    const auto FIRSTWINDOW = Desktop::viewState()->query().selector(first).runWindow();
    if (!FIRSTWINDOW)
        return {.success = false, .error = "First window not found"};
    if (Fullscreen::controller()->isFullscreen(FIRSTWINDOW))
        return {.success = false, .error = "Can't swap with fullscreen window"};
    const auto SECONDWINDOW = Desktop::viewState()->query().selector(second).runWindow();
